...
src/
  ...
  components/
    site/           全ページ共通（SiteHeader/SiteFooter/PageHero/CtaBand/FixedCta/DiagnosisPopup/DiagnosisApp/
                    SectionHead/FaqList/LeadForm/CorporateForm/RatioCycle/TeacherFilter/Icon/ArrowCircle）
                    ...
                    　'talk'=対話図（同）／'target'=絞り込み照準（/priceワケ）。自動再生なし（2026-09-11廃止）：初期は✗側で
                    　止まり、「フォーカス」タブ右上の赤ペン文字「ここをタップ！」＋その真下に描かれる矢印（現れる→描く→キープ→
                    　消える→描き直しの3秒周期）＋タブ自体の紺の波紋で誘導、最初のタップで誘導が消える。
                    　切替時は要素が時間差で遷移する振り付き
                    ...
                    ※PriceSimulator=料金シミュレータ（/price用。本棚の教科書をタップすると「6,578円×n教科（月4n回）」と月額合計を表示）
...
- **図解の器のルール（2026-08-15統一）**: 状態を切り替える図解（Before/Afterスライダー・タブ切替）だけが枠付きカード（✗=グレー地⇄◯=淡青地+紺枠）。それ以外の図解（静的図・自動アニメ・タップ小物）は方眼ノートカードに直置きし、パネルで囲まない
- **仮データの扱い**: 徒歩分数・教室座標・他塾比較金額・FAQ一部は仮。差し替えまで「※仮」表記を消さない（一覧は docs/design/README.md 末尾の公開前TODO）。保護者の声は2026-08-12に掲載確定済み（アンケート整合確認済み）
